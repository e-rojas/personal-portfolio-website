...
export default function Cards() {
  ...
  return (
      ...
      {cardsInfo.map((element, index) => {
        return <Card sumTotal={sumTotal} name={element.name} key={index} price={element.price} image={element.img} />
      })}
    </div>
    </div>
  )
...
const Card = props => {
  ...
  return (
      <div
      className={active ? " card alert alert-info" : " card alert alert-secondary"}
        onClick={handleClickCard}
          style={{ flexGrow: "3", flexBasis: "23rem",  color: 'black', cursor: 'pointer' }} >
      <div className="row">
        <div className="col-4">
                <img style={{width:'100px',height:'100px'}} className="img-thumbnail" alt='product' src={props.image} />
